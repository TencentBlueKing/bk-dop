<template>
    <div class="wrapper">
        ...
        <bk-dialog v-model="deleteCluster.primary.visible"
            ...
            @confirm="deleteEsData(deleteCluster.info)"
            title="集群信息删除任务">
        ...
        <bk-table id="mytable" style="margin-top: 15px;"
            ...
            <bk-table-column label="集群名称" prop="cluster_name">
                <template slot-scope="props">
                    <a v-bk-tooltips.right="'点击跳转到cerebro'" :href="cerebroUrl(props.row.cluster_name)" target="_blank" class="buttonText">{{props.row.cluster_name}}</a>
                </template>
            </bk-table-column>
...
<script>
    ...
    export default {
        ...
        created () {
            this.getCc()
            this.getEsData(this.formData)
        },
        methods: {
            cerebroUrl: function (cluster) {
                const url = window.AJAX_URL_PREFIX + 'es/cerebro/' + cluster
                return url
            },
            bklogUrl: function () {
            ...
            kibanaUrl: function (cluster) {
                const url = window.AJAX_URL_PREFIX + 'es/kibana/' + cluster
                return url
            },
            openKibana: function (cluster) {
                const url = window.AJAX_URL_PREFIX + 'es/kibana/' + cluster + '/'
                window.open(url)
            },
            handleInfo (config) {
            ...
            getClusterDetail (row) {
                this.$router.push(
                    {
                        path: 'nodeinfo', query: { 'row': row }
                    }
                )
            },
            deleteDialog (row) {
                if (row.cluster_status !== '集群变更中') {
            ...
            async deleteEsData (param) {
                try {
                    this.textLoading = true
                    const res = await this.$store.dispatch('es/deleteEsData', param)
                    if (res.code === 0) {
            ...
            async changeHandle (value) {
                ...
                this.textLoading = true
                this.getEsData({ 'selectedAppList': appIdList })
            },
