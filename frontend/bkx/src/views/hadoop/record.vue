    ...
    export default {
        ...
        methods: {
            ...
            openSideslider (row) {
                ...
                this.getHadoopTaskRecordDetail(row)
                // 定时器，每5秒刷新任务状态
            ...
            async getTaskData (param) {
                try {
                    const res = await this.$store.dispatch('hadoop/getHadooptaskRecord', param, { fromCache: true })
            ...
            async getHadoopTaskRecordDetail (row) {
                try {
                    this.textTaskLoading = true
                    const param = { 'pipeline_id': row.pipeline_id }
                    const res = await this.$store.dispatch('hadoop/getHadooptaskDetail', param, { fromCache: true })
            ...
            async optask (Id, opType) {
                try {
                    const res = await this.$store.dispatch('hadoop/opTask', { 'id': Id, 'op_type': opType })
                    if (res.code === 0) {
