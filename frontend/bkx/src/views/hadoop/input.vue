        ...
        <div>
            <bk-sideslider :is-show.sync="defaultSettings.isShow" :quick-close="true" :width="800">
                ...
                <div class="p20" slot="content">
                    ...
                    <bk-alert style="margin-top: 10px;" type="info" title="1> 目前hadoop集群只支持手动录入的方式。"></bk-alert>
                    ...
                    <bk-alert style="margin-top: 10px;" type="info" title="4> 录入的集群目前只对录入的节点进程信息做检测，其他特性暂不检测，自由度高。"></bk-alert>
            ...
            <bk-container :col="12">
                <bk-form :label-width="180" :model="formData" :rules="rules" ref="validateForm1">
                    <bk-row>
                        <bk-col :span="12">
                            <bk-alert type="info">
                                <bk-button slot="title" text @click="defaultSettings.isShow = true">查看Hadoop集群录入说明指引请点击</bk-button>
                            </bk-alert>
                        </bk-col>
                    </bk-row>
                    ...
                    <bk-row>
                        ...
                        <bk-col :span="12">
                            <div class="home-card-layout">
                                ...
                                <div class="card-title">录入集群基本信息</div>
                                <div style="width: 800px">
                                    ...
                                    <bk-form-item label="集群启动账号" :required="true" :property="'cluster_user'" :error-display-type="'normal'">
                                        <bk-input v-model="formData.cluster_user" placeholder="输入待录入集群启动进程账号"></bk-input>
                                        <p class="mt5 mb0 f12" slot="tip"></p>
                                    </bk-form-item>

                                    <bk-form-item label="集群安装目录" :required="true" :property="'base_dir'" :error-display-type="'normal'">
                                        <bk-input v-model="formData.base_dir" placeholder="请输入待集群组件的安装目录，所有组件必须存放在启动账号有权限的同一目录下"></bk-input>
                                        <p class="mt5 mb0 f12" slot="tip"></p>
                                    </bk-form-item>
                                    <bk-form-item label="ssh远程端口" :required="true" :property="'ssh_port'" :error-display-type="'normal'">
                                        <bk-input v-model="formData.ssh_port" placeholder="请输入待录入集群机器远程端口号"></bk-input>
                                        <p class="mt5 mb0 f12" slot="tip"></p>
                                    </bk-form-item>
                    ...
                    <bk-row>
                        <bk-col :span="12">
                            <div class="home-card-layout">
                                ...
                                <div style="width: 800px">
                                    <bk-form-item label="是否有部署ZK集群">
                                        <bk-button
                                            theme="primary"
                                            :outline="true"
                                            icon="plus"
                                            class="mr10"
                                            @click="isInputZk = true">
                                            添加ZK信息
                                        </bk-button>
                                        <bk-button
                                            theme="danger"
                                    ...
                                    <bk-form-item label="ZK 访问端口" v-show="isInputZk" :property="'zookeeperport'" :error-display-type="'normal'">
                                        <bk-input v-model="formData.zookeeperport" placeholder="请输入待录入zookeeper集群对外访问端口">
                                            <p class="mt5 mb0 f12" slot="tip"></p>
                                        </bk-input>
                                    </bk-form-item>
                                    <bk-form-item label="ZK 节点信息" v-show="isInputZk">
                                        <bk-input type="textarea" v-model="formData.zookeepernode" placeholder="请输入待录入Zookeeper的节点IP列表,多个可以用英文逗号或者空格隔开"></bk-input>
                                    </bk-form-item>
                            ...
                            <div class="home-card-layout">
                                ...
                                <div style="width: 800px">
                                    ...
                                    <bk-form-item label="hdfs副本数量" :required="true" :property="'repl_number'" :error-display-type="'normal'">
                                        <bk-input v-model="formData.repl_number" placeholder="输入副本数量"></bk-input>
                                    </bk-form-item>
                                    ...
                                    <bk-form-item label="是否有配置SNN节点">
                                        <bk-button
                                            theme="primary"
                                            :outline="true"
                                            icon="plus"
                                            class="mr10"
                                            @click="isInputSNN = true">
                                            添加SNN节点信息
                                        </bk-button>
                                        <bk-button
                                            theme="danger"
                                    ...
                                    <bk-form-item label="hdfs白名单文件名称" v-show="isInputInfo" :property="'hdfs_includes'" :error-display-type="'normal'">
                                        <bk-input v-model="formData.hdfs_includes" placeholder="请输入hdfs的白名单配置文件名称，填写绝对路径"></bk-input>
                                        <p class="mt5 mb0 f12" slot="tip"></p>
                                    </bk-form-item>
                                    <bk-form-item label="hdfs黑名单文件名称" v-show="isInputInfo" :property="'hdfs_excludes'" :error-display-type="'normal'">
                                        <bk-input v-model="formData.hdfs_excludes" placeholder="请输入hdfs的黑名单配置文件名称，填写绝对路径"></bk-input>
                                        <p class="mt5 mb0 f12" slot="tip"></p>
                                    </bk-form-item>
                                </div>
                            </div>
    ...
    export default {
            ...
            return {
                ...
                rules: {
                    ...
                    zookeeperport: [
                        {
                            validator: function (val) {
                                const regex = /^([0-9]|[1-9]\d{1,3}|[1-5]\d{4}|6[0-4]\d{3}|65[0-4]\d{2}|655[0-2]\d|6553[0-5])$/
                                if (val === '' || val.match(regex)) {
                                    return true
                                }
                            },
        ...
        created () {
            ...
            if (this.$route.query.row) {
                ...
                this.formData.base_dir = querydata.base_dir
                this.formData.repl_number = querydata.replication_number
                this.formData.ssh_port = querydata.ssh_port
                this.formData.hostname_list = querydata.hostname_list
                this.formData.is_create_bk_monitor = querydata.is_create_bk_monitor.toString()
                if (querydata.hdfs_includes !== '' || querydata.hdfs_excludes !== '') {
                    this.formData.hdfs_includes = querydata.hdfs_includes
                    this.formData.hdfs_excludes = querydata.hdfs_excludes
                ...
                if (querydata.standbynamenode.length !== 0) {
                    this.formData.standbynamenode = querydata.standbynamenode
                    this.isInputSNN = true
                }
                if (querydata.journalnode.length !== 0) {
                    this.formData.journalnode = querydata.journalnode
                    this.isInputJN = true
                }
                if (querydata.zookeepernode.length !== 0) {
                ...
                if (querydata.resourcemanager.length !== 0 || querydata.nodemanager.length !== 0) {
                    ...
                    this.isInputYarn = true
                }
            }
        },
        methods: {
            colseYarnInfo () {
                this.isInputYarn = false
                this.formData.resourcemanagernode = ''
                this.formData.nodemanagernode = ''
            },
            closeZkInfo () {
                this.isInputZk = false
                this.formData.zookeepernode = ''
                this.formData.zookeeperport = ''
            },
            closeInfo () {
                this.isInputInfo = false
                this.formData.hdfs_includes = ''
                this.formData.hdfs_excludes = ''
            },
            closeSnnInfo () {
                this.isInputSNN = false
                this.formData.standbynamenode = ''
            },
            closeJnInfo () {
                this.isInputJN = false
                this.formData.journalnode = ''
            },
            closeDialog () {
                this.$router.push(
                    {
