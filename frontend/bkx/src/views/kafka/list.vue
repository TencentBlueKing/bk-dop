<template>
    <div class="example1-wrapper">
        ...
        <bk-dialog v-model="deleteCluster.primary.visible"
            ...
            @confirm="deleteKafkaData(deleteCluster.info)"
            title="集群信息删除任务">
            <p>业务名称 : {{ deleteCluster.info.app }}</p>
            <p>集群名称 : {{ deleteCluster.info.cluster_name }}</p>
            <p style="color: red">删除集群的相关信息,并回收监控信息(不回收进程)，该操作不可逆！是否确认?</p>
        </bk-dialog>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async deleteKafkaData (param) {
                try {
                    this.textLoading = true
                    const res = await this.$store.dispatch('kafka/deleteKafkaData', param)
                    if (res.code === 0) {
            ...
            async changeHandle (value) {
                ...
                this.textLoading = true
                this.getKafkaData({ 'selectedAppList': appIdList })
            },
