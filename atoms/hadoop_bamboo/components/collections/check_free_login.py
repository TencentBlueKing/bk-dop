...
'''
@summary: 检测集群每个节点的用户免认证是否成功
@usage:
'''


class CheckFreeLogin(GeneralService):
    ...
    def execute(self, data, parent_data):
        ...
        kwargs = {
            ...
            "task_name": f"{cluster_name}集群各节点测试免认证",
            "script_content":
                get_script('hadoop_bamboo/components/collections/script_templates/check_free_login.sh'),
            "script_param": get_script_param([cluster_user, ssh_port, get_host_str(target_ips, hosts_info)]),
...
class CheckFreeLoginComponent(Component):
    name = __name__
    code = 'check_free_login_action'
    bound_service = CheckFreeLogin
