...
class InstallKafkaMonitorService(GeneralService):
    def execute(self, data, parent_data):
        ...
        broker_port = str(act_info['broker_port'])
        metric_port = str(act_info['metric_port'])

        kwargs = {
            "bk_biz_id": app_id,
            "bk_username": bk_username,
            "script_content": get_script('kafka_bamboo/components/collections/script_templates/init_monitor.sh'),
            "script_param": get_script_param([bk_data_id, access_token, broker_port, metric_port, target_ips_str]),
...
class InstallKafkaMonitorComponent(Component):
    name = __name__
    code = 'install_kafka_monitor_action'
    bound_service = InstallKafkaMonitorService
