...
class InstallKafkaService(GeneralService):
    def execute(self, data, parent_data):
        ...
        broker_str = act_info['broker_str']
        version = act_info['version']

        kwargs = {
            "bk_biz_id": app_id,
            "bk_username": bk_username,
            "script_content": get_script('kafka_bamboo/components/collections/script_templates/install_kafka.sh'),
            "script_param": get_script_param([broker_str, version]),
...
class InstallKafkaComponent(Component):
    name = __name__
    code = 'install_kafka_action'
    bound_service = InstallKafkaService
